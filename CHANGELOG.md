...
- README images of the grid picker and the hover preview, generated by
  `npm run generate-screenshots` from `media/picker.css`'s own metrics, the Dark
  Modern palette and real `IconIndex` results — so they track the code instead
  of going stale. The hover image is rendered from the `MarkdownString` the real
  `IconHoverProvider` returns, driven through the test suite's `vscode` stub.
  All three are renderings rather than screen captures.
...
[0.2.1]: https://github.com/iftekharweb/react-icons-inserter/releases/tag/v0.2.1
