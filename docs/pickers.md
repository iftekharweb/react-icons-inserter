...
![The grid picker](https://raw.githubusercontent.com/iftekharweb/react-icons-inserter/main/media/screenshot-grid.png)

> That image is generated by `npm run generate-screenshots` from this page's
> subject — `media/picker.css` metrics, Dark Modern colours and real
> `IconIndex` results — so it tracks the code rather than a stale capture.
